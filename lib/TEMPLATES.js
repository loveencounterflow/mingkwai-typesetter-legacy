...
(function() {
  ...
  this.doc_table = function(doc, style) {
    ...
    return render((function(_this) {
      return function() {
        var auto_space_chr, block_space_chr, cells_per_line, free_cell_chr, lines_per_page, ref, ref1, x1, xc, y1, yc;
        auto_space_chr = doc.auto_space_chr, block_space_chr = doc.block_space_chr, free_cell_chr = doc.free_cell_chr, cells_per_line = doc.cells_per_line, lines_per_page = doc.lines_per_page;
        ref = MKTS.get_next_xy(doc), x1 = ref[0], y1 = ref[1];
        ref1 = MKTS.xy_from_idx(doc, doc['idx']), xc = ref1[0], yc = ref1[1];
        switch (style) {
          case 'plain':
            return TABLE('.doc-table', function() {
              var i, j, k, ref2, ref3, ref4, x, y;
              TH();
              for (x = i = 0, ref2 = cells_per_line; 0 <= ref2 ? i < ref2 : i > ref2; x = 0 <= ref2 ? ++i : --i) {
                TH(x);
              }
              TH();
              for (y = j = 0, ref3 = y1; 0 <= ref3 ? j <= ref3 : j >= ref3; y = 0 <= ref3 ? ++j : --j) {
                if (y > 0 && y % lines_per_page === 0) {
                  TR(function() {
                    var k, ref4, results;
                    results = [];
                    for (x = k = 0, ref4 = cells_per_line + 1; 0 <= ref4 ? k < ref4 : k > ref4; x = 0 <= ref4 ? ++k : --k) {
                      results.push(TD('.separator'));
                    }
                    return results;
                  });
                }
                TR(function() {
                  var cell, cell_txt, clasz, k, ref4;
                  TH(y);
                  for (x = k = 0, ref4 = cells_per_line; 0 <= ref4 ? k < ref4 : k > ref4; x = 0 <= ref4 ? ++k : --k) {
          ...
          case 'sized':
            return TABLE('.doc-table', function() {
              ...
              for (y = j = 0, ref3 = y1; 0 <= ref3 ? j <= ref3 : j >= ref3; y = 0 <= ref3 ? ++j : --j) {
                ...
                TR(function() {
                  var Q, cell, cell_txt, clasz, k, ref4, ref5, size;
                  TH(y);
                  for (x = k = 0, ref4 = cells_per_line; 0 <= ref4 ? k < ref4 : k > ref4; x = 0 <= ref4 ? ++k : --k) {
                    cell = MKTS._get(doc, [x, y], void 0);
                    if (cell === MKTS.blockade) {
                      continue;
                    }
                    cell_txt = MKTS._rpr_cell(doc, cell);
                    size = (ref5 = cell != null ? cell['size'] : void 0) != null ? ref5 : 1;
                    Q = {};
                    if (size !== 1) {
