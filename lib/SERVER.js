// Generated by CoffeeScript 1.9.1

/*
#===========================================================================================================



 .d8888b.  8888888888 8888888b.  888     888 8888888888 8888888b.
d88P  Y88b 888        888   Y88b 888     888 888        888   Y88b
Y88b.      888        888    888 888     888 888        888    888
 "Y888b.   8888888    888   d88P Y88b   d88P 8888888    888   d88P
    "Y88b. 888        8888888P"   Y88b d88P  888        8888888P"
      "888 888        888 T88b     Y88o88P   888        888 T88b
Y88b  d88P 888        888  T88b     Y888P    888        888  T88b
 "Y8888P"  8888888888 888   T88b     Y8P     8888888888 888   T88b



#===========================================================================================================
 */

(function() {
  var Htmlparser, MKTS, TEMPLATES, TEXT, TRM, TYPES, XNCHR, alert, app, badge, debug, get_doc_updater, help, info, layout, port, rpr, server, sio, update_doc, urge, warn, whisper;

  TEXT = require('coffeenode-text');

  TYPES = require('coffeenode-types');

  TRM = require('coffeenode-trm');

  rpr = TRM.rpr.bind(TRM);

  badge = '明快排字机/SERVER';

  info = TRM.get_logger('info', badge);

  alert = TRM.get_logger('alert', badge);

  debug = TRM.get_logger('debug', badge);

  warn = TRM.get_logger('warn', badge);

  urge = TRM.get_logger('urge', badge);

  whisper = TRM.get_logger('whisper', badge);

  help = TRM.get_logger('help', badge);

  Htmlparser = (require('htmlparser2')).Parser;

  XNCHR = require('./XNCHR');

  MKTS = require('./main');

  TEMPLATES = require('./TEMPLATES');

  app = (require('express'))();

  server = (require('http')).Server(app);

  sio = (require('socket.io'))(server);

  port = 3000;

  layout = TEMPLATES.layout();

  get_doc_updater = function() {
    var chr_count, chr_idx, chrs, doc, last_size, nl_state, self;
    chrs = XNCHR.chrs_from_text("(畢昇發明活字印刷術)\n\n宋沈括著夢溪筆談卷十八記載\n(版印書籍唐人尚未盛為之)\n自馮瀛王始印五經已後典籍皆為版本\n(慶歷中有布衣畢昇又為活版)");
    chr_count = chrs.length;
    chr_idx = null;
    doc = null;
    nl_state = 'none';
    last_size = 1;
    self = function(command, event_emitter) {
      var chr, done, handler;
      switch (command) {
        case 'play':
          while (true) {
            self('next', event_emitter);
            if (chr_idx % 5 === 0) {
              break;
            }
          }
          break;
        case 'new':
        case 'reset':
          handler = function(observee, action, target, name, value) {
            var target_txt;
            if (action !== 'get') {

              /* `JSON.stringify target` strangely causes an `illegal access` error: */
              target_txt = rpr(target);
              if (event_emitter != null) {
                event_emitter.emit('change', observee, action, target_txt, name, value);
              }
              return null;
            }
          };
          chr_idx = 0;
          doc = MKTS.new_observable_document(handler);
          break;
        case 'next':
          done = false;
          while (!done) {
            if (chr_idx % chr_count === 0) {
              MKTS.advance_page(doc);
            }
            chr = chrs[chr_idx % chr_count];
            chr_idx += 1;
            if (chr !== '\n') {
              nl_state = 'none';
            }
            switch (chr) {
              case '\n':
                switch (nl_state) {
                  case 'none':
                    nl_state = 'pending';
                    break;
                  case 'pending':
                    MKTS.advance_line(doc, last_size);
                    nl_state = 'ignore';
                    break;
                  case 'ignore':
                    null;
                    break;
                  default:
                    throw new Error("unknown newline state " + (rpr(nl_state)));
                }
                break;
              case '(':
                MKTS.set_size(doc, 2);
                MKTS.compress(doc);
                done = true;
                break;
              case ')':
                MKTS.set_size(doc, 1);
                done = true;
                break;
              default:
                last_size = doc['size'];
                MKTS.put(doc, chr);
                done = true;
            }
          }
          break;
        default:
          warn("ignored MKTS command: " + command);
      }
      return doc;
    };
    return self;
  };

  update_doc = get_doc_updater();

  sio.on('connection', function(socket) {
    var doc, render;
    doc = null;
    urge("a user connected");
    socket.on('disconnect', function() {
      return warn('user disconnected');
    });
    socket.on('playback', function(command) {
      switch (command) {
        case 'next':
          return render(command);
        case 'reset':
          return render(command);
        case 'play':
          return render(command);
        default:
          return warn("ignored client event: playback/" + command);
      }
    });
    render = function(command) {
      if (doc == null) {
        doc = update_doc('new', socket);
      }
      doc = update_doc(command);
      sio.emit('new-table', 'plain', TEMPLATES.doc_table(doc, 'plain'));
      return sio.emit('new-table', 'sized', TEMPLATES.doc_table(doc, 'sized'));
    };
    render('new');
    return null;
  });

  app.get('/', (function(_this) {
    return function() {
      return function(request, response) {
        response.writeHead(200, {
          'Access-Control-Allow-Origin': '*'
        });
        response.write(layout);
        return response.end();
      };
    };
  })(this)());

  this.serve = function() {
    return server = server.listen(port, function() {
      var host, ref;
      ref = server.address(), host = ref.address, port = ref.port;
      return help("明快排字机 listening at http://" + host + ":" + port);
    });
  };

  if (module.parent == null) {
    this.serve();
  }

}).call(this);
