...
(function() {
  ...
  this.new_document = function(settings) {
    var R, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7;
    if (settings == null) {
      settings = {};
    }
    R = {
    ...
    R['keep_x_grid'] = (ref = settings['keep_x_grid']) != null ? ref : false;
    R['cells_per_line'] = (ref1 = settings['cells-per-line']) != null ? ref1 : 8;
    R['lines_per_page'] = (ref2 = settings['lines-per-page']) != null ? ref2 : 6;
    R['auto_space_chr'] = (ref3 = settings['auto-space-chr']) != null ? ref3 : '\u3000';
    R['blockade_chr'] = (ref4 = settings['blockade-chr']) != null ? ref4 : '＃';
    R['free_cell_chr'] = (ref5 = settings['free-cell-chr']) != null ? ref5 : '〇';
    R['layout'] = (ref6 = settings['layout']) != null ? ref6 : 'rows';
    R['direction'] = (ref7 = settings['direction']) != null ? ref7 : 'ltr';
  ...
  this._new_block = function(me, content, content_key) {
    var R, size;
    size = me.size;
    R = {
  ...
  this.put = function(me, content, content_key) {
    var blockade_chr, cells, dx, dy, i, idx, j, ref, ref1, ref2, size, x0, y0;
    if (content_key == null) {
      content_key = null;
    }
    this.advance_chr(me);
    idx = me.idx, size = me.size, cells = me.cells, blockade_chr = me.blockade_chr;
    ref = this._get_xy(me), x0 = ref[0], y0 = ref[1];
    this._set(me, idx, this._new_block(me, content, content_key));
    for (dx = i = 0, ref1 = size; 0 <= ref1 ? i < ref1 : i > ref1; dx = 0 <= ref1 ? ++i : --i) {
      for (dy = j = 0, ref2 = size; 0 <= ref2 ? j < ref2 : j > ref2; dy = 0 <= ref2 ? ++j : --j) {
        if ((dx === dy && dy === 0)) {
          continue;
        }
  ...
  this._new_border_info = function(me, type, size, position) {
    var R;
    if (type !== 'box') {
    ...
    if (position !== 'lone' && position !== 'start' && position !== 'middle' && position !== 'end') {
      throw new Error("unknown type " + (rpr(type)));
    }
    R = {
  ...
  this._new_border = function(me, start, stop) {
    var R;
    R = {
  ...
  this.start_box = function(me) {
    var block, cells, idx, size, type;
    idx = me.idx, cells = me.cells;
    block = cells[idx];
    if ((type = TYPES.type_of(block)) !== 'MINGKWAI/TYPESETTER/block') {
      throw new Error("unable to use borders on content of type " + (rpr(type)));
    }

    /* TAINT must finish previous box */
    size = block.size;
    block['border'] = this._new_border_info(me, 'box', size, 'start');
    me['border_block_size'] = size;
    return me;
  };

  this.box = function(me) {
    var block, cells, idx, size, type;
    idx = me.idx, cells = me.cells;
    block = cells[idx];
    if ((type = TYPES.type_of(block)) !== 'MINGKWAI/TYPESETTER/block') {
    ...
    /* TAINT must finish previous box */
    size = block.size;
    block['border'] = this._new_border_info(me, 'box', size, 'lone');
    me['border_block_size'] = null;
    return me;
  };

  this.get_borders = function(me, page_idx) {
    var R, _, block, border, cells, col0, col1, entry, i, idx, len, pcr, position, row0, row1, size, type, xy;
    if (page_idx == null) {
      page_idx = null;
    }
    R = [];
    ...
    for (idx = i = 0, len = cells.length; i < len; idx = ++i) {
      block = cells[idx];
      if ((TYPES.type_of(block)) !== 'MINGKWAI/TYPESETTER/block') {
        continue;
      }
      border = block.border;
      if (border == null) {
      ...
      pcr = this.pcr_from_xy(me, xy);

      /* TAINT should implement `idx_from_pcr` */
      if ((page_idx != null) && page_idx !== pcr[0]) {
        continue;
      }
      type = border.type, position = border.position, size = border.size;
      switch (type) {
        case 'box':
          switch (position) {
            case 'lone':
              _ = pcr[0], col0 = pcr[1], row0 = pcr[2];
              col1 = col0 + size;
              row1 = row0 + size;
              entry = [this._new_border(me, [page_idx, col0, row0], [page_idx, col1, row0]), this._new_border(me, [page_idx, col1, row0], [page_idx, col1, row1]), this._new_border(me, [page_idx, col0, row1], [page_idx, col1, row1]), this._new_border(me, [page_idx, col0, row0], [page_idx, col0, row1])];
              break;
            default:
              warn("unknown border type " + (rpr(type)));
          }
          break;
        default:
          warn("unknown border type " + (rpr(type)));
  ...
  this._advance = function(me, mode, size) {
    var idx, last_x, last_y, p, p1, ref, x, y, y1;
    idx = me.idx;
    if (idx < 0) {
      return me;
    }
    ...
    last_y = null;
    while (true) {
      ref = this._get_xy(me), x = ref[0], y = ref[1];
      if (mode === 'line') {
        if (y > y1) {
          break;
        }
  ...
  this.advance_chr = function(me) {
    ...
    while (true) {
      ...
      on_grid_line = (modulo(this._get_y(me), size)) === 0;
      cell_is_free = cells[me['idx']] === void 0;
  ...
  this.compress = function(me) {
    var auto_space_chr, blank_count, blockade_chr, cell, cell_count, cells, cells_per_line, chr_count, d, doc_idx, doc_x, doc_y, dx, dy, height, i, idx, idx0, idx1, j, k, l, len, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, size, tmp_cell, tmp_cells, tmp_cells_per_line, tmp_idx, width, x, x0, x1, y, y0, y1;
    ...
    ref = this.xy_from_idx(me, idx), x = ref[0], y = ref[1];
    ref1 = this._get_grid_line_ys(me, y, size), y0 = ref1[0], y1 = ref1[1];
    ...
    blank_count = modulo(chr_count, height);
    if (blank_count > 0) {
    ...
    tmp_cells = [];
    for (doc_y = i = ref2 = y0, ref3 = y1; ref2 <= ref3 ? i <= ref3 : i >= ref3; doc_y = ref2 <= ref3 ? ++i : --i) {
      for (doc_x = j = ref4 = x0, ref5 = x1; ref4 <= ref5 ? j <= ref5 : j >= ref5; doc_x = ref4 <= ref5 ? ++j : --j) {
    ...
    for (d = k = 0, ref6 = blank_count; 0 <= ref6 ? k < ref6 : k > ref6; d = 0 <= ref6 ? ++k : --k) {
      tmp_cells.push(auto_space_chr);
    }
    me['idx'] = idx0 + tmp_cells_per_line - 1;
    for (tmp_idx = l = 0, len = tmp_cells.length; l < len; tmp_idx = ++l) {
      tmp_cell = tmp_cells[tmp_idx];
      ref7 = this.xy_from_idx(null, tmp_idx, tmp_cells_per_line), dx = ref7[0], dy = ref7[1];
  ...
  this.xy_from_idx = function(me, idx, cells_per_line) {
    ...
    return [modulo(idx, cells_per_line), Math.floor(idx / cells_per_line)];
  };
  ...
  this.pcr_from_xy = function(me, xy, size) {
    var cells_per_line, col, direction, layout, lines_per_page, page_idx, ref, ref1, row, width, x, y;
    if (size == null) {
      size = 1;
    }
    ...
    y = modulo(y, lines_per_page);
    if (layout === 'rows') {
      ref = [x, y], col = ref[0], row = ref[1];
      width = cells_per_line;
    } else {
      ref1 = [y, x], col = ref1[0], row = ref1[1];
  ...
  this.walk = function(me, handler) {
    var blockade_chr, cells, cells_per_line, content, content_txt, data, event, i, idx, last_page_idx, len, lines_per_page, page_idx, pcr, size, type, xy;
    ...
    for (idx = i = 0, len = cells.length; i < len; idx = ++i) {
      ...
      if (page_idx !== last_page_idx) {
        if (last_page_idx != null) {
          handler(null, ['page-end', last_page_idx]);
        }
  ...
  this.$assemble_html_events = function(settings) {
    ...
    return P1.remit((function(_this) {
      return function(event, send, end) {
        var attributes, chr, i, input, len, match, name, ok, ref, size, tail, type;
        if (is_first_event) {
          start_size(1);
          send(['doc', me]);
        ...
        if (event != null) {
          ...
          switch (type) {
            case 'text':
              advance_now();
              ref = XNCHR.chrs_from_text(tail[0]);
